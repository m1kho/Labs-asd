namespace lvl3
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("=== ПОЧАТКОВИЙ ВМІСТ ДЕРЕВА (INORDER) ===");
            tree.Inorder();

            string uniCity = "Київ";
            Console.WriteLine($"\n=== ПОШУК ТА ВИДАЛЕННЯ: Студенти 1-го курсу, які приїхали не з міста {uniCity} ===");
            ...
            if (found.Count > 0)
            {
                foreach (var s in found)
                {
                    ...
                    Console.WriteLine($"Видаляємо: {s.LastName} {s.FirstName} (Квиток: {s.StudentTicket:D6})");
                    tree.Delete(s.StudentTicket);
                }
                Console.ResetColor();
